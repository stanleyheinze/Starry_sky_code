...
for i = 1:length(fileList)
    ...
    elseif strcmp(type,'Bar')
        iBarcw(c_BAR,:) = circshift(deg_cw, zero_adjust + 90); % adjust for change in stimulus starting position
        iBarccw(c_BAR,:) = circshift(deg_ccw, zero_adjust + 90); % adjust for change in stimulus starting position
        BGBar(c_BAR) = response.bg_mean_cw;
        BGsemBar(c_BAR) = response.bg_SEM_cw;
        c_BAR = c_BAR + 1;
    elseif strcmp(type,'Gradient bar')
        iGrdcw(c_GRD,:) = circshift(deg_cw, zero_adjust + 90); % adjust for change in stimulus starting position
        iGrdccw(c_GRD,:) = circshift(deg_ccw, zero_adjust + 90); % adjust for change in stimulus starting position
